...
class TicketCommentUpdated extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function toMail(object $notifiable): MailMessage
    {
        ...
        return (new MailMessage)
            ->subject(__('Comment Updated on :ticket', ['ticket' => $ticketName]))
            ->greeting(__('Hello :name,', ['name' => $notifiable->name ?? __('there')]))
            ->line(__(':author updated a comment on ticket ":ticket".', [
                'author' => $author,
                'ticket' => $ticketName,
            ]))
            ->line(Str::limit((string) $this->comment->body, 160))
            ->action(__('View Ticket'), url(TicketResource::getUrl('view', ['record' => $this->ticket])))
            ->line(__('Please review the latest discussion when you have a moment.'));
    }
