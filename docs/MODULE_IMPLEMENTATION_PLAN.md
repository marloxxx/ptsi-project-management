...
## Phase 0 – Foundation
- [ ] Align composer dependencies (Filament 4, Filament Shield, Socialite, Excel, Octane, tab layout plugin)
- [ ] Configure environment variables (.env.example) for OAuth, mail, queues, and external portal
...
- **Tasks**
  - [ ] Recreate migrations using clean architecture folder conventions.
  - [ ] Implement repositories/services under `Domain` and `Application` namespaces.
  - [ ] Seed base data (roles, default statuses, priorities).
- **Testing**
  - [ ] Feature tests covering migrations & seeders (ensure roles and permissions seeded correctly).
...
- **Tasks**
  - [ ] Configure Socialite in `config/services.php` and environment stubs.
...
- **Tasks**
  - [ ] Implement ticket service (creation, assignment, status transitions, history entries).
